    ...
    <div className="flex-1 flex flex-col overflow-y-auto">
      <header className="pt-4 pb-2 px-6 flex flex-col items-center">
        <div className="w-full flex justify-between items-center mb-4">
          <div className="w-10" />
          <h1 className="text-2xl font-black tracking-tighter text-charcoal">clutter</h1>
          <button onClick={() => setPhase(6)} className="text-sm font-medium text-grey hover:text-charcoal transition-colors" data-testid="button-skip">Skip</button>
        </div>
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="text-center mb-2">
          ...
          <p className="text-grey text-[15px] leading-relaxed px-2">
            We handle the heavy lifting so you never have to visit a storage unit again.
          </p>
        </motion.div>
      </header>
      ...
      <div className="flex-1 px-5 pt-4 pb-4 flex flex-col">
        <div className="relative flex-1">
          {/* Old Way card — fades out at phase 3 */}
          <motion.div
            initial={{ opacity: 1 }}
            animate={{ opacity: phase >= 3 ? 0 : 1 }}
            transition={{ duration: 0.8, ease: "easeInOut" }}
            className={phase >= 3 ? "absolute inset-x-0 top-0 pointer-events-none" : "relative"}
          >
            <OldWayCard phase={phase} />
          </motion.div>

          {/* Clutter Way card — fades in at phase 3 */}
          <motion.div
            initial={{ opacity: 0 }}
...
function OldWayCard({ phase }: { phase: number }) {
  return (
    <div className="bg-white p-5 rounded-2xl border border-grey-light shadow-sm">
      <div className="flex items-center gap-3 mb-10">
        ...
        <div>
          <h3 className="text-[19px] font-bold text-charcoal tracking-tight leading-none mb-1.5">The Old Way</h3>
          <p className="text-[10px] font-bold text-grey uppercase tracking-wider">Traditional self-storage</p>
        </div>
      </div>
      ...
      <div className="relative w-full overflow-hidden mt-2 mb-2">
        ...
        <AnimatePresence>
          {phase >= 2 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="flex flex-col items-center mt-2">
              <div className="w-[70%] relative h-5 flex items-center justify-center">
                ...
                <motion.div animate={{ x: ["-80px", "80px", "-80px"], rotateY: [0, 0, 180, 180, 0] }} transition={{ duration: 6, ease: "easeInOut", repeat: Infinity }} className="relative z-10 bg-white px-1.5 text-orange-400">
                  <Truck size={14} />
                </motion.div>
              </div>
              <span className="text-[8px] font-bold text-orange-400 mt-1 uppercase tracking-wider">Trips to and from storage</span>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
...
function ClutterWayCard({ phase }: { phase: number }) {
  return (
    <div className="relative p-5 rounded-2xl border overflow-hidden bg-gradient-to-br from-teal-light to-[#e6f4f1] border-[#d3eae5]">
      <div className="flex items-center gap-3 mb-6">
        <div className="p-2.5 rounded-xl shadow-sm bg-teal text-white">
          <CheckCircle2 size={22} strokeWidth={2.5} />
        </div>
        <div>
          <h3 className="text-[19px] font-bold tracking-tight leading-none mb-1.5 text-teal">The Clutter Way</h3>
          <p className="text-[10px] font-bold uppercase tracking-wider text-teal/60">Full-service storage</p>
        </div>
      </div>
          ...
          {phase >= 4 && (
            <motion.div initial={{ opacity: 0, height: 0, scale: 0.9 }} animate={{ opacity: 1, height: "auto", scale: 1 }} className="bg-white rounded-2xl p-4 border border-white shadow-sm overflow-hidden">
              ...
              <div className="relative h-12 flex items-center justify-between px-6 bg-mist rounded-xl border border-grey-light/30">
                ...
                <div className="relative z-10 flex flex-col items-center">
                  <div className="bg-white p-2 border-2 border-teal rounded-[10px] text-teal shadow-sm">
