...
export default function Screen8Lead({ goTo }: Props) {
  ...
  const showDelivery = state.branch === 'B1' || state.branch === 'B3';
  const isB3 = state.branch === 'B3';

  const canContinue = firstName.trim() && lastName.trim() && email.includes('@') && phone.length >= 7 && pickupAddress.trim() && (!showDelivery || deliveryAddress.trim());

  function handleContinue() {
    setState({ firstName, lastName, email, phone, pickupAddress, deliveryAddress });
    if (state.branch === 'B2' || state.branch === 'B4') {
      goTo('screen-success');
    } else {
      goTo('screen-addons');
    }
          ...
          <AnimatePresence>
            {showDelivery && (
              ...
              >
                ...
                />
                {isB3 && (
                  <div className="mt-3 p-3 bg-flex-light rounded-xl border border-flex/20">
                    <p className="text-xs text-grey leading-relaxed">
                      Long-distance delivery will be handled by our partner Flex. A specialist will call within 24 hours.
                    </p>
                  </div>
                )}
              </motion.div>
